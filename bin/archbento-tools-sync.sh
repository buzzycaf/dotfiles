...
# Prefer installed tools.env; fallback to repo tools.env when running from a checkout
TOOLS_ENV_REPO="$REPO_DIR/tools.env"
# Installed source-of-truth (copied by dotfiles/install)
TOOLS_ENV_INSTALLED="$HOME/.local/state/archbento/tools.env"

if [[ -f "$TOOLS_ENV_INSTALLED" ]]; then
  TOOLS_ENV="$TOOLS_ENV_INSTALLED"
elif [[ -f "$TOOLS_ENV_REPO" ]]; then
...
tmp_hypr="$(mktemp)"
{
  ...
  echo "# AUTOGENERATED from $TOOLS_ENV — DO NOT EDIT"
  echo "# ====================================================================="
  echo
  for key in "${ORDER[@]}"; do
    val="${KV[$key]:-}"
